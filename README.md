...
> 💡 **特别提醒**
>
> 本分支为 **个人自用定制版本**。在保持与原版核心功能同步的同时，会根据个人需求添加一些可能违背原版“极简/设计初衷”的功能。
>
> **请在选择前明确您的需求：**
> 1. **优先推荐原版**：如果您追求最纯粹的 Ech0 体验，请优先选择 [原版](https://github.com/lin-snow/Ech0)。
> 2. **按需使用**：仅在您确实需要本分支提供的特定功能（如视频支持、Live Photo 等）时才考虑使用。
> 3. **随时切回**：本分支提供了便捷的[双向切换/数据同步方案](#migration-guide)，您可以随时无损切回原版。
> 4. **同步策略**：本分支会第一时间跟进上游更新。若自定义逻辑与上游代码冲突，将优先保留上游逻辑。
> 5. **性能损耗声明**：为了确保与原版的兼容性以及同步便利，代码中包含了一些额外的兼容逻辑（如针对旧版数据库表的同步方案），这可能会带来极其微小的性能开销。如果您对性能有极致追求，请知悉并慎重选择。
...
### 📱 PWA 与 SEO 增强
- **PWA 信息自动同步** — 自动将站点标题、描述和 Logo 同步至 PWA 配置，安装体验更统一
- **Logo 智能裁剪与动态生成** — 上传 Logo 时自动居中裁剪，后端 `/api/icon` 端点按需生成各尺寸图标（192/512/180/favicon），适配 maskable 安全区域
- **社交分享内容动态化** — 深度优化 OpenGraph 协议，将链接分享至 QQ、Telegram 或 X (Twitter) 等平台时，可动态显示对应的正文摘要与媒体预览，卡片更美观且具辨识度
- **智能页面标题** — 精准匹配当前浏览状态，在动态详情页、搜索关键词或日期筛选页自动切换标题，识别更清晰
- **快捷入口 (Shortcuts)** — 长按应用图标可快速跳转至首页、Hub、设置等常用功能（支持 Android/Desktop）
- **Web Share Target** — 从其他应用（如微博、浏览器）分享内容时，可直接选择 Ech0 作为目标，快速创建新 Echo
- **原生分享增强** — 深度适配 Web Share API，点击分享时优先调用系统原生分享面板，在 PWA 模式下提供更接近原生应用的交互体验
- **全方位离线支持** — 支持离线访问已缓存的内容；离线状态下发布的纯文字内容会自动进入同步队列，待网络恢复后自动发布；实时感知网络状态，离线时禁止媒体上传并给予明确提示
- **系统通知推送** — 支持收件箱、待办事项、Hub 更新的系统通知，使用分类专属彩色图标（📩 / 📋 / ✨），可直接在通知中完成"设为已读"或"完成任务"等操作
- **应用角标计数** — 自动同步未读数到应用图标角标（收件箱 + 待办 + Hub 更新）
